// file: components/plate/PlateFileModal.js
'use client';

import { useEffect, useMemo, useState } from 'react';
import {
  createPlateFileInFolder,
  createPlateFolder,
  deletePlateFile,
  deletePlateFolder,
  loadPlateFS,
...
export default function PlateFileModal({
  open,
  mode, // 'save' | 'open'
  onClose,
    ...
    >
      <div className="w-full max-w-4xl rounded-xl bg-white shadow-2xl overflow-hidden ring-1 ring-black/10">
        <div className="flex items-center justify-between border-b px-4 py-3 bg-white">
          <div className="font-semibold">{mode === 'save' ? '切板：保存' : '切板：開く'}</div>
          ...
          <div className="p-3 space-y-3 bg-white">
            <div className="flex items-end justify-between gap-3">
              <div>
                ...
                <div className="text-[11px] text-gray-500">※ 切板専用のブラウザ保存（localStorage）</div>
              </div>
                ...
                <div className="flex gap-2 items-end">
                  ...
                  <button
                    ...
                    onClick={() => {
                      ...
                      const next = createPlateFileInFolder(fs, activeFolder.id, nm, payload);
                      commit(next);
                      setFileName('');
                      alert('保存しました');
                    }}
                      ...
                      <div className="flex gap-2 justify-end">
                        {mode === 'open' ? (
                          <button
                            ...
                            type="button"
                            onClick={() => {
                              if (!confirm(`「${f.name}」を開きますか？（現在の作業は上書きされます）`)) return;
                        ...
                        {mode === 'save' ? (
                          <button
                            ...
                            onClick={() => {
                              ...
                              const next = overwritePlateFile(fs, activeFolder.id, f.id, nm.trim() || f.name, payload);
                              commit(next);
                              alert('上書き保存しました');
                            }}
                          >
            ...
            <div className="flex gap-2">
              <button
                ...
                type="button"
                onClick={() => {
                  if (!confirm('切板の保存データを全消去しますか？')) return;
                  const next = { folders: [] };
                  commit(next);
        ...
        <div className="border-t px-4 py-3 text-[11px] text-gray-500 bg-white">
          切板は切板で別保存です（3Dエディタ保存とは別）。将来ここを「サーバ保存」に差し替え可能です。
        </div>
      </div>
    </div>
  );
